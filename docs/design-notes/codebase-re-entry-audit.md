...
| blocks | `locations/tasks.py:133` (`commute_tick` → `schedule.py`) vs `gameworld/tasks.py:38-50` (`sun_phase_started` → `react_to_sun_phase` → `go_home`/`go_outside`) | Two independent systems decide where a character walks, with different triggers and different destination logic. Only one is scheduled (see §7), but both are live code. **Resolved: document, don't remove.** Confirmed via `progress_rpg/celery.py` that only `commute_tick` is on the beat schedule — `move_characters_tick` and `precompute_sun_times`/`schedule_sun_phase_tasks` are commented out (see §7), so the sun-phase → `go_home`/`go_outside` path is currently unreachable, not a live parallel system. It stays a §7 dead-code candidate rather than something to remove here — removing reachable-looking code that isn't actually dead-in-the-VCS-sense (still imported, tested, one flag away from live) is a bigger call than a readability pass should make unilaterally. |
...
| slows | `api/views.py` (APIView + `ViewSet` + `@action`), `progression/views.py` (`ModelViewSet` + queryset mixins), `locations/views.py` (bare `APIView` + `ReadOnlyModelViewSet`), `gameplay/views.py` (`ViewSet` + `@action`) | Four view idioms for CRUD-ish endpoints, with permission and queryset scoping done differently in each. **Resolved: document, don't unify.** Naming the four patterns is the whole fix here — unifying permission and queryset scoping across four apps is a bigger, riskier change than a readability pass should make; each idiom is internally consistent for its own app, and no single "correct" idiom stands out enough to justify the migration cost. Revisit only if a fifth idiom is about to be introduced. |
| slows | `frontend/src/api/*.ts` vs direct `apiFetch` callers | A dedicated API layer exists (`api/tasks.ts`, `api/notes.ts`, …) *and* seven modules bypass it: `hooks/useActivityTimer.ts`, `useBootstrapGameData.ts`, `useOnboarding.ts`, `useTutorialSteps.ts`, `useMaintenanceStatus.ts`, `components/MaintenanceWatcher.tsx`, `components/TutorialModal/TutorialModal.tsx`. **Correction:** `MaintenanceWatcher.tsx` doesn't call `apiFetch` at all — it registers global `setMaintenanceHandler`/`setNetworkErrorHandler` callbacks that the shared `apiFetch` in `utils/api.ts` invokes on 503/network-error. That's configuring the shared layer's error handling, not bypassing it, so it isn't a real instance of this finding; six modules, not seven. **Resolved:** five new `api/*.ts` modules added, one per resource (`activityTimers.ts`, `gameData.ts`, `onboarding.ts`, `tutorial.ts`, `maintenance.ts`), matching the existing one-file-per-resource convention. All six call sites updated to import from them instead of calling `apiFetch` directly — same request shapes, same endpoints, no behaviour change. |
